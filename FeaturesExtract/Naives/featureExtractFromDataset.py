...
#file to store words
f = open('../../Dataset/Datasets-after-feature-extraction/Naives-Bayes/anomalousCombinedWordsss.txt', 'w')
...
def filter(load):
    ...
    p=re.compile('\+|\&|\=|\%3C|\%3E|\<|\>|\%253C|\%253E|\%252F')
    # extract attributes from header body
    if (checkForMethod(load)==0):
        print >> f, urllib.unquote(str(p.sub(' ',load)))
    ...
    if (checkForGet(load)):
           try:
               ...
               print >> f,urllib.unquote(str(p.sub(' ',load[start+1:end])))
           except ValueError:
               return ""
...
fromfile("../../Dataset/Original Datasets/anomalousTrafficTest.txt")
